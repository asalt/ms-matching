...
def make_fragments(peptide, types=('b', 'y'), maxcharge=1): # from https://pyteomics.readthedocs.io/en/latest/examples/example_msms.html
    """
...
def do_match(peptide_sequence, raw_mz, error_tol=20, error_tol_units="ppm", verbose=True, **kwargs):
    ...
    if error_tol_units == "ppm":
        error_tol = error_tol * 1e-6 * raw_mz["m/z array"].mean() # convert ppm to Da, approximating the mean m/z as the average observed value
    ...
    b_fragments = list(make_fragments(peptide_sequence, types='b', maxcharge=parent_ion_charge-1))
    y_fragments = list(make_fragments(peptide_sequence, types='y', maxcharge=parent_ion_charge-1))

    # how does this work?
    # could be rewritten to be more clear
    b_matches = [ x for x in b_fragments if any( abs( x - y ) < error_tol for y in mz_array ) ]
    y_matches = [ x for x in y_fragments if any( abs( x - y ) < error_tol for y in mz_array ) ]
    # how do we get the intensities for the matched fragments?


    # solution
    assert len(mz_array) == len(intensity_array), "m/z array and intensity array must be the same length"
    ...
    for y_fragment in y_fragments:
        for mz, intensity in zip(mz_array, intensity_array):
            if abs(y_fragment - mz) < error_tol:
                if verbose:
                    print(f"Match found for y fragment {y_fragment} at m/z {mz} with intensity {intensity}")
                b_fragment_intensities.append(intensity)
                break # break because we have found the match
